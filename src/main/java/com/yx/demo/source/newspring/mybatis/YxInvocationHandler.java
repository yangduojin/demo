...
public class YxInvocationHandler implements InvocationHandler {
    String biz = null;
    public YxInvocationHandler(String biz) {
        this.biz = biz;
    }
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (select != null) {
            ...
            if (method.getAnnotatedReturnType().getType() instanceof ParameterizedType) {
                ...
                String json = JSONArray.toJSONString(YxSqlDeal.mockSelectList(biz));
                ...
                String json = JSONArray.toJSONString(YxSqlDeal.mockSelect(biz));
                ObjectMapper objectMapper = new ObjectMapper();
        ...
        if (insert != null) {
            ...
            YxSqlDeal YxSqlDeal = new YxSqlDeal();
            YxSqlDeal.insert(s, Arrays.toString(args));
            return 1;
        }
        Gray gray = method.getAnnotation(Gray.class);
        if (gray != null && result !=null) {
            if(!StringUtils.isEmpty(gray.biz())) {
                biz = gray.biz();
            }
            Map<String, JSONObject> jsons = YxSqlDeal.gray(biz);
            if (method.getAnnotatedReturnType().getType() instanceof ParameterizedType) {
                ParameterizedType pt = (ParameterizedType) method.getAnnotatedReturnType().getType();
                Class<?> c = (Class<?>) pt.getActualTypeArguments()[0];
